...
func _draw() -> void:
	...
	var r: float = size.y / 2.0 - 2.0

	# Yellow selection ring drawn first (underneath the circle)
	if is_selected:
		draw_circle(c, r + 4.0, Color(1.0, 0.90, 0.0, 0.95))

	var col: Color = data.owner_faction.color if data.owner_faction != null else Color.GRAY
	draw_circle(c, r, col)
	draw_arc(c, r, 0.0, TAU, 32, Color(1, 1, 1, 0.8), 1.5)

	# First letter of faction name as a simple marker
	var letter: String = data.owner_faction.faction_name.left(1).to_upper() if data.owner_faction != null else "?"
	var font: Font = ThemeDB.fallback_font
	var fs: int = int(r * 0.95)
	var tw: float = font.get_string_size(letter, HORIZONTAL_ALIGNMENT_LEFT, -1, fs).x
